        ...
        <el-tab-pane label="短信登录" name="sms">
          ...
          <el-form-item prop="userCaptcha" class="userCaptcha-main" label="验证码">
            <el-input v-model="userForm.userCaptcha" type="text" :prefix-icon="Lock" size="large" auto-complete="off" placeholder="验证码" placeholder-style="color: blue" :style="{ fontSize: '16px' }">
              <template #suffix>
                <el-link v-if="userForm.disableclick">{{ userForm.time }}秒后发送</el-link>
                <el-link :underline="false" v-else :disabled="userForm.disableclick" @click="sendYzm()" :style="{ 'pointer-events': userForm.disableclick ? 'none !important' : 'auto' }">发送验证码</el-link>
              </template>
            </el-input>
          </el-form-item>
          <el-form-item class="submit">
