...
<script setup lang="ts">
  ...
  const activeIndex = ref('0'); // Set the default index here
...
const uniqueNavMenuList = computed(() => {
  const seen = new Set();
  return navMenuList.value.filter(item => {
    if (seen.has(item.navName)) {
      return false;
    }
    seen.add(item.navName);
    return true;
  });
});
